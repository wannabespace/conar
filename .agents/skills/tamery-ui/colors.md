...
`::selection` is `bg-primary/30 text-primary` everywhere, with one inversion in `globals.css`: on a **primary-colored surface** it flips to `bg-white/90 text-primary`, because a blue tint over a blue fill erases the text. That override cannot key on the `bg-primary` class alone — a surface is often painted by a parent's variant (`Bubble`'s `*:data-[slot=bubble-content]:bg-primary`), which leaves **no class on the element holding the text**, so the selector list also matches the slot. Any new solid-primary surface painted that way needs adding there; a class-based selector silently misses it and the bug reads as selection not working at all.
