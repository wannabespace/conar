...
- `--accent` is the **hover surface**: always `bg-accent`, never a per-call-site colour, never an alpha tint. The light levels span only a few points of lightness, so one absolute value cannot sit the same distance from all three — globals.css therefore re-declares `--accent` per level surface (light only), so every level hovers the same step past itself and call sites stay unchanged. A tint on a solid surface reads as grime over a near-white card, changes strength with whatever is behind it, and lets content under a floating control show through it.
...
- **Rows on the translucent canvas are the exception: they hover to `bg-foreground/5` and press or latch to `/10`.** Under vibrancy the canvas takes the wallpaper's tint and lightness, so a solid grey reads as a dead patch whose contrast drifts with the desktop — a tint keeps a fixed step and picks up the tint like everything around it (native sidebars do the same). Only rows and ghost row-buttons (`variant="ghost-row"`); controls on the canvas keep their solid level-3 fill and `bg-accent` hover.
- A control inside a row that already hovers to `bg-accent` hovers and opens to a foreground tint (`bg-foreground/5`–`/10`), never `bg-accent`: the kit's `bg-accent` vanishes on the hovered row. Kit `Button` and `SelectTrigger` carry it as `variant="ghost-row"`; `AppMenuButton` takes it through `render`.
