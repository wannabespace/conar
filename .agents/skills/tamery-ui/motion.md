...
- **Virtualized reorder** (tables tree pin): motion `layout` can't work with tanstack-virtual — stable `getItemKey` (row id, not index) + `motion.li` with `initial={false} animate={{ y: virtualRow.start }}`. `y` changes only on reorder, never during scroll. Ref: `sidebar/tables-list.tsx`.
