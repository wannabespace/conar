...
- Row heights: menu/list rows `h-7`, connection-list rows `h-9` (icon `size-5`, color bar `h-4`), tab bar `h-8`, toolbars `h-11`–`h-12`, chips `h-5` (filter) / `h-6`. Data-table header has **no fixed height** — padding + line heights land on 32/44px naturally; `--table-header-height` in globals must match, and hairlines stay non-layout (`inset-ring`) — a real `border` adds pixels.
