...
- **Monaco SQL takes its language id from `sqlLanguageIds`, never `"sql"`** — the four dialect languages are registered in `entities/connection/sql-language.ts` (tokens from `@tamery/sql`, completion and diagnostics bound per model); a `sql` model gets stock grammar and nothing else. Its themes are hand-mapped from shiki's palettes so editor and `CodeBlock` read as one — keep the two in step.
