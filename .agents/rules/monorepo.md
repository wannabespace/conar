...
**Default: `agent-browser` CLI via Bash** — not the user's Chrome. Deliberately not an MCP server or project dependency; install globally (`npm install -g agent-browser && agent-browser install`). `agent-browser open <url>`, then `snapshot`, `click @ref`, `type`, `console`, `errors`, `screenshot`. Its Chrome trusts the portless CA and keeps a logged-in session across runs (verified 2026-08-26) — don't re-test before reaching for another browser.
...
**The embedded/sandboxed browser pane cannot load portless HTTPS hosts** — every subresource is cancelled with `net::ERR_BLOCKED_BY_CLIENT` (block sits in the pane's own request layer; already ruled out TLS/CSP/service workers — don't re-debug). Bare `http://localhost:<port>` works there.
