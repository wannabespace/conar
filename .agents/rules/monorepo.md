...
- `motion-panels` — the resizable pane layout mechanics (framework-agnostic core plus a React adapter, animated with motion, no third-party layout library). An external npm dependency, developed in its own repo (`~/Projects/Own/motion-panels`), not a workspace package. It is not published yet, so `pnpm-workspace.yaml` carries the wiring in one place: the catalog pins the version it will have and an `overrides` entry redirects it to `link:../../Own/motion-panels/motion-panels` (override `link:` paths resolve from the workspace root; catalogs reject the `link:` protocol). Consuming packages just say `catalog:`. Install therefore needs that sibling checkout, and tamery sees the package's `dist`, so a change there needs `pnpm --filter motion-panels run build` before it shows up here. It ships unstyled and has no `cn`: the kit wraps it as `ResizableGroup`/`ResizablePanel`/`ResizableSeparator` (`packages/ui/components/custom/resizable.tsx`, importing `motion-panels/react`) and the app imports those. Behaviour and rules in the `tamery-ui` skill (patterns → Pane splits, motion → Pane folds).
