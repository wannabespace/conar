...
- `@tamery/vite-inline-html` (`packages/vite-inline-html`) — Vite plugins that fill `index.html` markers (dev and build): the root entry inlines bundled IIFE scripts and is framework-free, the `/react` entry server-renders React components. Knows nothing about shells, boot, or CSS — every target is passed from the app's `vite.config.ts`.
