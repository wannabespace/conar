...
- Two stream starters because of the Redis pointer's takeover rule: `restartChatStream` claims with **NX** and returns `null` to the loser (racing mounts answer a question once); `startChatStream` overwrites plainly (a send must never be blocked by a stale pointer).
