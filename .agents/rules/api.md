...
`routers/chats/v1/` = frozen AI SDK implementation; its wire format is what shipped desktop builds parse — **never port it**. `routers/index.ts` keeps `ai.chat` as alias of `chats.v1.chat` for those clients; no app in this repo ships a chat UI. v1 self-contained: AI SDK packages + `zod` are deps of `apps/api` alone, exist **only** for it — nothing new may reach for them.

Every other AI procedure runs on `@tanstack/ai` through adapters in `apps/api/lib/ai.ts` — Anthropic only. TanStack AI has no cross-provider failover; hand-rolled one not worth owning — route through a gateway adapter if that changes.
