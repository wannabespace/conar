...
| Memoization | React Compiler on in `apps/app` + `apps/main` vite configs, reaches `packages/*` (workspace sources resolve outside `node_modules`). No `useMemo`/`useCallback` — derive inline; `react/jsx-no-constructed-context-values` off for same reason. Caveat: compiler skips any component calling TanStack Virtual's `useVirtualizer` directly (incompatible-library bailout, enforced by `react/incompatible-library` lint) — never import it; use `@tamery/ui/hooks/use-virtualizer` wrapper (isolates bailout behind `'use no memo'`, returns compiler-safe values: `virtualItems`/`totalSize` snapshots for render, stable `measure`/`scrollToIndex` for imperative calls). Verify a suspected bailout by running `babel-plugin-react-compiler` on the file with a `logger`, not by reading source. |
