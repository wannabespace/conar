...
- **DDL is raw `sql` with `sql.id`, not Kysely's schema builders.** Kysely covers table, index, schema, type and view only — no function, procedure, trigger or policy, no `ALTER INDEX`, no two actions in one `ALTER TABLE` — and `alterType().renameValue()` types its argument to `never` for names that are runtime strings. Where a builder does exist it splits a name on `.`: `alterTable('public.sales.2024')` compiles to `alter table "public"."sales"`, silently the wrong table, and `createType('public.my.enum')` throws. `sql.id(schema, name)` brackets each part instead. Rows, table drop/rename and every catalog list stay on the builders.
