...
- **A save that needs more than one statement runs them in one `db.transaction()`** — Postgres and SQL Server roll the whole swap back, so a failed `CREATE` leaves the old object standing. MySQL commits DDL implicitly and rolls nothing back, so it is the one engine whose drop-then-create still warns the user. Its trigger and routine recreates read `SHOW CREATE` before the drop and replay it if the create fails, and pass the saved `DEFINER` so a save never hands the object to the editing account.
...
- **A transaction opener's isolation level and access mode travel as Kysely's `TransactionSettings`** into each executor's `beginTransaction`, which spells its own opener (SQL Server's `BEGIN TRAN` carries neither, MySQL's only the access mode). An executor that ignores them silently turns `BEGIN READ ONLY` into a writable transaction.
