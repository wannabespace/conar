...
- **The runner asks for `resultSets`** (`RunOptions` on `execute`/`executeTransaction`): each engine answers with `ResultSet[]` — columns in order with duplicate names kept, rows as arrays, the affected-row count for a write, and `truncated` once rows pass `maxRows`. Every other caller gets row objects as before. A statement with several sets (a SQL Server batch, a MySQL `CALL`, a Postgres text of several statements) answers with one set each.
