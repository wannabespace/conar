...
- Name for the thing, not the mechanism: the object is the subject and the method the verb, so the call site reads as a sentence — `chatStream.resume(chatId)`, `activeStream.release(chatId, streamId)`. A name that describes the data structure or the plumbing (`pointer`, `map`, `handler`, `chatTurn`) tells a reader nothing about what it holds; if the concept can't be named, the grouping is wrong. No stutter — the object already carries the noun, so `chatStream.claimStream` and `activeStream.streamKey` repeat it; drop the repeat from the method.
