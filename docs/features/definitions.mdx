...
<Steps>
  <Step title="Open the Definitions tab">The sidebar shows the categories available for your database type.</Step>
  <Step title="Choose a category">Click a category (e.g. **Indexes**) to list that object type for the selected schema.</Step>
  <Step title="Switch schemas">Use the **Schema** dropdown at the top of the sidebar.</Step>
  <Step title="Inspect an object">Click an item to expand its details inline — index columns, function bodies, policy rules, etc.</Step>
</Steps>
