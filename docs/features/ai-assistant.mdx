...
<Note>
  The assistant only runs read-only queries through its database tool. It never
  runs `INSERT`, `UPDATE`, `DELETE`, or `DROP` on its own — data changes go
  through the table browser or SQL editor.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card
    title="Filters from English"
    icon="filter"
    href="/features/table-browser"
  >
