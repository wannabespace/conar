...
<Note>
  The desktop app is recommended for most users. It runs a local proxy, so
  localhost and private-network databases work without extra setup.
</Note>
...
<Tip>
  To reach local databases from the web app, run `tamery proxy` (see the
  [CLI](/cli/overview)) and point a connection at it.
</Tip>
