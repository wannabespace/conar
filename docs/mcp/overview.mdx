...
<Note>
  The MCP integration is in active development and not yet generally available.
  This page will be expanded with setup steps, available tools, and examples as
...
<Warning>
  Treat API keys like passwords. Anyone with a key can act on your behalf. Store
  keys securely and revoke any key you no longer use from the same account page.
</Warning>
