...
export const chatPersist = {
  deleteMessage: async (data: {
    ...
    const [deleted] = await db
      ...
      .where(
        and(
          eq(chatsMessages.id, data.messageId),
          eq(chatsMessages.chatId, data.chatId)
        )
      )
  ...
  loadMessages: async (data: { chatId: string; userId: string }) => {
    const rows = await db
      .select({
