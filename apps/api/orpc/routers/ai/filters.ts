...
import { addDays, differenceInSeconds, endOfDay, format, startOfDay } from 'date-fns'
...
const redisUsage = {
  ...
  increment: async (userId: string) => {
    ...
    await redis.expire(key, differenceInSeconds(endOfDay(now), now))
    return value
  },
}
    ...
    if (!context.subscription) {
      ...
      if (usage >= FREE_AI_FILTERS_USAGE_DAILY_LIMIT) {
        throw errors.FORBIDDEN({
          message: 'You have reached the free AI daily usage limit. Please subscribe to a Pro plan to continue using AI features.',
          data: {
            remaining: 0,
            max: FREE_AI_FILTERS_USAGE_DAILY_LIMIT,
            resetAt: startOfDay(addDays(new Date(), 1)),
          },
        })
      }
    }
