  ...
  .handler(async function ({ input, context }) {
    ...
    const [updatedItems, newItems, existingIds] = await Promise.all([
            ...
            .where(
              and(
                ...
                or(
                  ...input.map((query) =>
                    and(
                      eq(queries.id, query.id),
                      gte(queries.updatedAt, addSeconds(query.updatedAt, 1))
                    )
                  )
                )
              )
      ...
      db
        .select()
        .from(queries)
        .where(and(eq(queries.userId, context.user.id), notInArray(queries.id, inputIds))),
      db
        .select({ id: queries.id })
        .from(queries)
        .where(and(eq(queries.userId, context.user.id), inArray(queries.id, inputIds)))
