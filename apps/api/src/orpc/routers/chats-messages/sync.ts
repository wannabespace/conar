  ...
  .handler(async function ({ input, context }) {
    ...
    const [updatedItems, newItems, existingIds] = await Promise.all([
            ...
            .where(
              and(
                ...
                or(
                  ...input.map((message) =>
                    and(
                      eq(chatsMessages.id, message.id),
                      gt(chatsMessages.updatedAt, addSeconds(message.updatedAt, 1))
                    )
                  )
                )
              )
      ...
      db
        ...
        .from(chatsMessages)
        .innerJoin(chats, eq(chatsMessages.chatId, chats.id))
        .where(and(eq(chats.userId, context.user.id), notInArray(chatsMessages.id, inputIds))),
      db
        .select({ id: chatsMessages.id })
        .from(chatsMessages)
        .innerJoin(chats, eq(chatsMessages.chatId, chats.id))
        .where(and(eq(chats.userId, context.user.id), inArray(chatsMessages.id, inputIds)))
