...
function RouteComponent() {
  ...
  return (
    <div className="mx-auto max-w-xl py-6">
      ...
      <Accordion type="multiple" value={expandedReleases} onValueChange={setExpandedReleases} className="space-y-6">
        {releases.map((release, index) => (
          <AccordionItem key={release.id} value={String(release.id)} className="border-none" disabled={!release.body}>
            <div className="mb-2">
              <AccordionTrigger className={cn(`py-0 hover:no-underline`)}>
                <div className="flex flex-col gap-1">
                  ...
                  {release.publishedAt && <p className="text-sm text-muted-foreground">Released on {format(new Date(release.publishedAt), 'MMMM dd, yyyy')}</p>}
                </div>
              </AccordionTrigger>
            </div>
            {release.body && (
