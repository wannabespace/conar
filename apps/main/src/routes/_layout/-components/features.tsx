...
function FeatureCard({ className, children, featureId, ...props }: FeatureCardProps) {
  return (
    <article className={cn('rounded-2xl border bg-card p-8', className)} data-feature={featureId} {...props}>
      {children}
    </article>
  )
}
...
function FeatureCardTitle({ title, description, icon, className }: FeatureCardTitleProps) {
  return (
    <header className={cn('flex flex-col', className)}>
      <h3 className={`mb-3 flex items-center gap-2 text-sm font-medium tracking-wide text-muted-foreground`}>
        {icon}
        {title}
      </h3>
...
function FilterItem({ name, operator, value, onRemove }: FilterItemProps) {
  return (
    <div className={`flex h-7 items-center overflow-hidden rounded-md border transition-colors dark:bg-input/30`}>
      <div className={`flex h-full items-center gap-1 px-2 text-xs font-medium text-foreground`}>
        <RiDatabase2Line className="size-3 text-primary/70" aria-hidden="true" />
        {name}
      </div>
      <Separator orientation="vertical" />
      <div className={`h-full px-2 py-0.5 font-mono text-xs text-muted-foreground`}>{operator}</div>
      <Separator orientation="vertical" />
      <div className={`max-w-20 truncate px-2 font-mono text-xs text-foreground sm:max-w-40 lg:max-w-60`}>{value}</div>
      <Separator orientation="vertical" className="h-7" />
      <button
        type="button"
        className={`flex h-full w-7 items-center justify-center transition-colors hover:bg-muted/50`}
        onClick={onRemove}
    ...
    <div className={`space-y-4 sm:space-y-6`}>
      ...
      <div className={`space-y-3 sm:space-y-4`}>
        <div className="relative w-full">
          <RiBardLine className={`pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground`} aria-hidden="true" />
          <Input
            className={`w-full pr-12 pl-10 text-sm focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20`}
          ...
          <Button
            variant="secondary"
            size="icon-xs"
            className={`absolute top-1/2 right-2 -translate-y-1/2 transition-colors hover:bg-primary hover:text-primary-foreground`}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" size="icon-xs" className={`transition-colors hover:bg-primary hover:text-primary-foreground`} aria-label="Add new filter">
            <RiAddLine className="size-3" aria-hidden="true" />
          </Button>
        </div>
        <div className="overflow-hidden overflow-x-auto rounded-lg border">
          <table className="w-full min-w-100 text-xs" aria-label="Filtered data results">
            <thead className="bg-muted/50">
              <tr>
                <th className={`p-2 text-left font-medium text-foreground sm:p-3`}>ID</th>
                <th className={`p-2 text-left font-medium text-foreground sm:p-3`}>Name</th>
                <th className={`p-2 text-left font-medium text-foreground sm:p-3`}>Price</th>
                <th className={`p-2 text-left font-medium text-foreground sm:p-3`}>Category</th>
              </tr>
            </thead>
            <tbody>
...
function UserMessage({ children, ...props }: MessageProps) {
  return (
    <div className="flex gap-3" {...props}>
      ...
      <div className={`max-w-[85%] rounded-lg bg-muted px-4 py-3 text-sm text-foreground`}>{children}</div>
    </div>
  )
}
...
function AIMessage({ children, ...props }: MessageProps) {
  return (
    <div className="flex justify-end gap-3" {...props}>
      <div className={`max-w-[85%] rounded-lg bg-primary px-4 py-3 text-sm text-primary-foreground shadow-sm`}>{children}</div>
      <div className={`flex size-7 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-medium text-white shadow-sm`}>AI</div>
    </div>
  )
}
    ...
    <div className={`space-y-4 sm:space-y-6`}>
      ...
      <div className={`space-y-3 sm:space-y-4`}>
        ...
        <AIMessage aria-label="AI response">
          ...
          <br />
          <code className={`mt-2 inline-block rounded-sm bg-primary-foreground/20 px-2 py-1 font-mono text-xs break-all`}>
...
function CloudSync() {
  return (
    <div className={`space-y-4 sm:space-y-6`}>
      ...
      />
      <div className={`space-y-3 sm:space-y-4`}>
        <div className={`flex items-center gap-3 rounded-xl border border-border/50 bg-linear-to-r from-muted/30 to-muted/50 p-3 sm:gap-4 sm:p-4`}>
          <div className={`flex size-10 shrink-0 items-center justify-center rounded-md bg-linear-to-br from-primary/20 to-primary/10 sm:size-12`}>
            <RiDatabase2Line className={`size-5 text-primary sm:size-6`} aria-hidden="true" />
          </div>
          ...
          <div className="flex shrink-0 flex-col items-end gap-1 text-xs">
            <span className={`font-medium text-green-600 dark:text-green-400`}>Synced</span>
            <span className={`hidden text-muted-foreground sm:block`}>without password</span>
          </div>
        </div>
        <div className={`flex items-center gap-3 rounded-xl border border-border/50 bg-linear-to-r from-muted/30 to-muted/50 p-3 sm:gap-4 sm:p-4`}>
          <div className={`flex size-10 shrink-0 items-center justify-center rounded-md bg-linear-to-br from-primary/20 to-primary/10 sm:size-12`}>
...
function DataActionCard({ icon, title, description, color }: DataActionCardProps) {
  ...
  return (
    <div className={cn(`cursor-pointer rounded-lg border bg-linear-to-br p-3 transition-all duration-300 hover:shadow-md sm:p-4`, colorClasses[color])}>
      <div className={`mb-2 flex items-center gap-2 sm:mb-3 sm:gap-3`}>
        <div className={cn(`flex size-6 shrink-0 items-center justify-center rounded-md sm:size-7`, bgColorClasses[color])}>
...
function ManageData() {
  return (
    <div className={`space-y-4 sm:space-y-6`}>
      ...
      <div className="mt-4 text-xs text-muted-foreground/70 italic">* Some features coming in nearest releases</div>
    </div>
  )
}
  ...
  return (
    <section aria-labelledby="features-heading" className={`py-8 sm:py-12 lg:py-16`}>
      <div className={`mb-12 text-center sm:mb-16`}>
        <h2 id="features-heading" className={`mb-3 text-center text-sm font-medium tracking-wide text-muted-foreground uppercase`}>
          Features
        </h2>
