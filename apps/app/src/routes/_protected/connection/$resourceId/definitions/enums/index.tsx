  ...
  return (
    <>
      ...
      <div className="mb-4 flex items-center gap-2">
        <SearchInput placeholder="Search enums" autoFocus value={search} onChange={(e) => setSearch(e.target.value)} onClear={() => setSearch('')} />
      ...
      <DefinitionsGrid loading={isPending}>
        {filteredEnums.length === 0 && <DefinitionsEmptyState title="No enums found" description="This schema doesn't have any enums defined yet." />}
              ...
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2 text-base">
                    {enumItem.metadata?.isSet ? <RiListIndefinite className="size-4 text-primary" /> : <RiListUnordered className="size-4 text-primary" />}
                        ...
                        <HighlightText
                          ...
                          render={({ html, matched }) => (
                            <motion.div layout {...MOTION_BLOCK_PROPS}>
                              <Badge variant="outline" className={cn('text-xs', matched && 'border-primary/30 bg-primary/10')}>
