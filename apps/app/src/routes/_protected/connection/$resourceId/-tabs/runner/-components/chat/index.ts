...
import type { AITools } from '@tamery/ai/tools'
import type { AppUIMessage } from '@tamery/ai/tools/helpers'
import { convertToAppUIMessage } from '@tamery/ai/tools/helpers'
    ...
    const chat = new Chat<AppUIMessage>({
      ...
      transport: {
        async sendMessages(options) {
          ...
          const runnerStore = runnerPageStore({
          ...
          return eventIteratorToStream(
            ...
            \`\`\`sql
            ${runnerStore.get().query.trim() || '-- empty'}
            \`\`\`
            `,
