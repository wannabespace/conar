...
const SchemaRow = ({
  ...
  return (
    <SidebarGroupLabel
      ...
      className="group hover:bg-accent/50 h-full w-full cursor-default gap-1 px-1.5"
    >
      <RiArrowRightSLine
        className={cn(
      ...
      <SidebarMenuButton
        ...
        render={
          <Link
            ...
            data-mask
            onDoubleClick={() =>
              addTab(connectionResource.id, row.schema, row.table.name)
            }
          />
        }
      >
          ...
          if (row.kind === 'schema') {
            ...
            rowContent = (
              <div className="flex h-full items-center">
                <Separator className={cn('bg-border mx-2 w-full', className)} />
              </div>
            )
          } else {
            rowContent = (
