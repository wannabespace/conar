...
export const BASE_GENERATORS = {
  ...
  'number.int': {
    label: 'Integer',
    category: 'Number',
    generate: () => faker.number.int({ max: 10000 }),
  },
  'number.float': {
    label: 'Float',
    category: 'Number',
    generate: () => faker.number.float({ max: 10000, fractionDigits: 2 }),
  },
  'number.bigInt': {
    label: 'Big Integer',
    category: 'Number',
    generate: () => String(faker.number.bigInt({ max: 9007199254740991n })),
  },
  'number.binary': {
    label: 'Binary',
    category: 'Number',
  ...
  'number.percentage': {
    label: 'Percentage',
    category: 'Number',
    generate: () => faker.number.float({ min: 0, max: 100, fractionDigits: 2 }),
  },
...
export function baseAutoDetectGenerator(name: string, type: string): keyof typeof BASE_GENERATORS {
  ...
  if (name.includes('currency')) return 'finance.currencyName'
  ...
  if (name.includes('filename')) return 'system.fileName'
  if (name.includes('filepath')) return 'system.filePath'
  if (name.includes('fileext') || name.includes('extension')) return 'system.fileExt'
