...
export const tablesAndSchemasType = type({
  schema: 'string',
  table: 'string',
  type: type.or(type.enumerated(...tableTypes), type.enumerated(...(tableTypes.map((t) => t.toUpperCase()) as Uppercase<(typeof tableTypes)[number]>[]))),
}).pipe(({ type, ...props }) => {
    ...
    query: {
      postgres: (db) =>
        db
          ...
          .where(({ eb, and, not }) => and([not(eb('n.nspname', 'like', 'pg_toast%')), not(eb('n.nspname', 'like', 'pg_temp%'))]))
