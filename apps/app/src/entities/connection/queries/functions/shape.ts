...
export const createFunctionStatements = ({
  ...
  return {
    ...
    mysql: sql`CREATE ${keyword} ${name}(${args})${returns}${behavior} ${body}`,
    ...
    postgres: sql`CREATE ${keyword} ${name}(${args})${returns} LANGUAGE ${sql.raw(shape.language)}${behavior}${security} AS ${sql.raw(`$tamery_body$${shape.body}$tamery_body$`)}`,
  }
}
