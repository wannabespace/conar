    ...
    >
      ...
      <span
        className={cn(
          ...
          schemaParam === row.name && 'text-sidebar-foreground',
        )}
      >
        {row.name}
      </span>
    ...
    <ContextMenu>
      <ContextMenuTrigger className="block h-full">
        <SidebarMenuButton
          isActive={isActive}
          className={`
            h-7 cursor-default rounded-md pl-2 text-sm
            text-sidebar-foreground
            hover:text-sidebar-foreground data-active:bg-primary
        ...
        <SidebarMenuAction
          ...
          className={cn('top-1! rounded-md', isActive && 'hover:bg-primary-foreground/20')}
          onClick={() => togglePinTable(connectionResource.id, row.schema, row.table.name)}
        >
          {row.pinned ? (
            <RiPushpinFill
              className={cn('size-3!', isActive ? 'text-primary-foreground' : 'text-primary')}
            />
          ) : (
            <RiPushpinLine
              className={cn(
            ...
            >
              {row.kind === 'schema' ? (
                ...
                <div className="flex h-full items-center">
                  <SidebarSeparator className="mx-2 w-full" />
                </div>
              ) : (
