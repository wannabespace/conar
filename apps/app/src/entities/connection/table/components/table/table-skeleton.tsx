  ...
  return (
    <div className={cn('relative w-full', className)}>
      {Array.from({ length: ROWS_COUNT }).map((_, rowIndex) => (
        <div
          key={rowIndex}
          className={`flex w-fit min-w-full border-b last:border-b-0`}
          style={{
        ...
        >
          ...
          {cols.map((column, index) => (
            <div
              key={index}
              className={`flex h-full shrink-0 items-center px-2 py-1 first:pl-4`}
              style={{
                width: `${column.size}px`,
              }}
            >
              <div className={`h-4 w-full shrink-0 animate-pulse rounded-sm bg-muted`} />
            </div>
          ))}
        </div>
      ))}
