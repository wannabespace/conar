...
import { Frame, FrameHeader, FramePanel, FrameTitle } from '@tamery/ui/components/frame'
...
function ValueCell({
  ...
  return (
    <div
      className={cn(
        `max-h-40 overflow-auto rounded-md border bg-muted/30 px-2 py-1 font-mono text-[0.7rem] wrap-break-word`,
...
export function DraftsReviewDrawer({
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} position="right">
      <DrawerPopup showCloseButton variant="inset" position="right" className="max-w-2xl">
        <DrawerHeader>
