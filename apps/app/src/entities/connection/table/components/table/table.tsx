...
function createSelectionCellRenderer(keys: string[]) {
  return function SelectionCellRenderer(props: TableCellProps) {
    return <SelectionCell keys={keys} {...props} />
  }
}

function createSelectionHeaderRenderer(keys: string[]) {
  return function SelectionHeaderRenderer(props: TableHeaderCellProps) {
    return <SelectionHeaderCell keys={keys} {...props} />
  }
}
...
function createTableHeaderCellRenderer(column: Column, handlers: ColumnHandlers) {
  return function TableHeaderCellRenderer(props: TableHeaderCellProps) {
    return <TableHeaderCell column={column} {...handlers} {...props} />
  }
}
      ...
      .map(column => {
        ...
        return {
          ...
          size:
            ...
            (column.foreign ? 25 : 0),
          header: createTableHeaderCellRenderer(column, handlers),
          cell: createTableBodyCellRenderer(column, connection.type, primaryColumns, handlers),
  ...
  const providerColumns = useMemo<ColumnRenderer[]>(() => {
    ...
    if (primaryColumns.length > 0) {
      result.push({
        ...
        cell: createSelectionCellRenderer(primaryColumns),
        header: createSelectionHeaderRenderer(primaryColumns),
        size: 40,
      })
    }
