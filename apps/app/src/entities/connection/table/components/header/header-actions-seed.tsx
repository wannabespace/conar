...
export function HeaderActionsSeed({ table, schema }: { table: string; schema: string }) {
  ...
  return (
    <Drawer open={open} onOpenChange={handleOpenChange} position="right">
      <Tooltip>
        <TooltipTrigger asChild>
          <DrawerTrigger render={<Button variant="secondary" size="icon" disabled={isPending} />}>
            <RiSeedlingLine />
          </DrawerTrigger>
        </TooltipTrigger>
        <TooltipContent>Seed data</TooltipContent>
      </Tooltip>
      <DrawerPopup showCloseButton variant="inset" position="right" className="max-w-xl">
        <DrawerHeader>
          <DrawerTitle>Seed Data</DrawerTitle>
          <DrawerDescription>
            Generate fake data for {schema}.{table}
          </DrawerDescription>
        </DrawerHeader>
        <DrawerPanel>
          {!subscription && (
