      ...
      <div className="flex shrink-0 flex-col gap-2 p-4 pb-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4">
            {!CONNECTION_TYPES_WITHOUT_SYSTEM_TABLES.includes(connection.type) && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Switch checked={showSystem} onCheckedChange={(value) => store.set((state) => ({ ...state, showSystem: value }) satisfies typeof state)} />
                </TooltipTrigger>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <RefreshButton variant="outline" size="icon" onClick={() => refetchTablesAndSchemas()} refreshing={isRefreshingTablesAndSchemas} />
              </TooltipTrigger>
