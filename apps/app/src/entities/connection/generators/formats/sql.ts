...
const buildColumnParts = (
  ...
  if (c.primaryKey) {
    ...
    if (dialect === ConnectionType.MySQL && /int|serial/iu.test(rawType)) {
      parts.push('AUTO_INCREMENT')
    }
    if (dialect === ConnectionType.MSSQL && /int/iu.test(rawType)) {
...
export const generateSchemaSQL = ({
  ...
  let schema = templates.sqlSchemaTemplate(
    quoteIdentifier(table, dialect),
    columnBlock
  )
