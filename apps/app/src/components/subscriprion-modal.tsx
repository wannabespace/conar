    ...
    <Dialog open={isSubscriptionDialogOpen} onOpenChange={setIsSubscriptionDialogOpen}>
      <DialogContent className={`max-w-md bg-linear-to-b from-primary/5 via-background to-background`}>
        <DialogHeader>
          ...
          <DialogDescription className="text-foreground">Subscribe to Pro to access this feature and unlock the full power of Conar.</DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <div className="flex items-start gap-3 rounded-lg bg-muted/50 px-6 py-2">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Your subscription directly supports our work and future development.</p>
            </div>
          </div>
        </div>
        <DialogFooter className={`flex-col gap-2 sm:flex-row`}>
          <Button variant="outline" onClick={() => setIsSubscriptionDialogOpen(false)} className={`w-full sm:w-auto`}>
            Maybe Later
          </Button>
          <Button className={`w-full sm:w-auto`} render={<a href={accountUrl} target="_blank" rel="noopener noreferrer" aria-label="Upgrade to Pro" />}>
            Upgrade to Pro
