...
export function SidebarButton({ className, active, ...props }: ComponentProps<'button'> & { active?: boolean }) {
  return <button type="button" className={cn(baseClasses, className, active ? activeClasses : inactiveClasses)} {...props} />
}

export function SidebarLink({ className, ...props }: LinkProps & Omit<ComponentProps<'a'>, 'children'>) {
  return <Link activeProps={{ className: activeClasses }} inactiveProps={{ className: inactiveClasses }} className={cn(baseClasses, className)} {...props} />
}
