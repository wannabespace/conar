import type { connectionResourceStoreType } from '.'
import { toast } from 'sonner'
import { connectionResourceStore } from '.'

export function addTab(id: string, schema: string, table: string, preview?: boolean) {
  const store = connectionResourceStore(id)
...
export function updateTabs(id: string, newTabs: typeof connectionResourceStoreType.infer['tabs']) {
  const store = connectionResourceStore(id)
...
export function setChatPosition(id: string, position: typeof connectionResourceStoreType.infer['layout']['chatPosition']) {
  const store = connectionResourceStore(id)
  store.setState(state => ({
    ...state,
    layout: {
