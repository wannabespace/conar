...
export const databasesCollection = createCollection(
  drizzleCollectionOptions({
    ...
    sync: async ({ write, collection }) => {
      ...
      const sync = await orpc.databases.sync(
      ...
      for (const item of sync) {
        if (item.type === 'insert') {
          ...
          if (
            item.value.syncType === SyncType.CloudWithoutPassword &&
            localPassword &&
            !cloudPassword
          ) {
    ...
    onInsert: async ({ transaction }) => {
      await Promise.all(
        transaction.mutations.map((m) =>
          orpc.databases.create({
            ...m.modified,
            connectionString: prepareConnectionStringToCloud(
              m.modified.connectionString,
              m.modified.syncType
            ),
          })
        )
