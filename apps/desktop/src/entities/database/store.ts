...
export function databaseStore(id: string) {
  ...
  const state = pageStoreType(Object.assign({}, defaultState, persistedState))
...
type LastOpenedPage = Extract<
  keyof FileRoutesById,
  `/(protected)/_protected/database/$id/${string}`
>

export function getDatabasePageId(routesIds: (keyof FileRoutesById)[]) {
  return routesIds.find((route) => route.includes('/(protected)/_protected/database/$id/')) as
    | LastOpenedPage
    | undefined
}
...
export function addTab(id: string, schema: string, table: string, preview?: boolean) {
  ...
  if (
    !store.state.tabs.find((tab) => tab.table === table && tab.schema === schema && !tab.preview)
  ) {
    store.setState(
      (prev) =>
        ({
          ...
          tabs:
            prev.tabs.map((tab) =>
              tab.table === table && tab.schema === schema ? { table, schema, preview: false } : tab
            ) ?? [],
        }) satisfies typeof prev
    )
  }
