...
export function prepareValue(value: unknown, type?: string, database?: typeof databases.$inferSelect): unknown {
  if (!type)
    return value
...
export async function getPrimaryKeyColumns(db: typeof databases.$inferSelect, schema: string, tableName: string): Promise<string[]> {
  try {
    const constraints = await queryClient.fetchQuery(
      databaseConstraintsQuery({ database: db }),
    )
...
export function buildInsertPayload(
  ...
  const valueParams = filteredColumnNames.map(colId =>
    prepareValue(raw[colId], columns.find(c => c.id === colId)?.type, database),
  )
