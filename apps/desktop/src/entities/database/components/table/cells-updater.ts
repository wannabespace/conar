export interface CellUpdaterFunction {
  (rowIndex: number, columnName: string, value: string | null): Promise<void>
}

export interface CellUpdaterOptions {
  setValue: (rowIndex: number, columnName: string, value: string | null) => void
  saveValue: (rowIndex: number, columnName: string, value: string | null) => Promise<void>
  getValue: (rowIndex: number, columnName: string) => string | null
}
...
export function createCellUpdater() {
  ...
  return ({ setValue, saveValue, getValue }: CellUpdaterOptions): CellUpdaterFunction => {
    return async (rowIndex: number, columnName: string, value: string | null) => {
