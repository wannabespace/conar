...
export const Route = createFileRoute('/(protected)/_protected/database/$id/table/')({
  validateSearch: type({
    'schema?': 'string',
    'table?': 'string',
  ...
  loader: ({ context, deps }) => {
    const store =
      deps.table && deps.schema
        ? createPageStore({ id: context.database.id, schema: deps.schema, table: deps.table })
        : null
...
function TableContent({
  ...
  table: string
  schema: string
  store: Store<typeof storeState.infer>
}) {
  ...
  const removeUnusedOrdersEvent = useEffectEvent(() => {
    ...
    const invalidOrderByKeys = Object.keys(store.state.orderBy).filter(
      (key) => !columnIds.includes(key)
    )
  ...
  return (
    <PageStoreContext value={store}>
      ...
      >
        <FiltersProvider columns={columns ?? []} filtersGrouped={SQL_FILTERS_GROUPED}>
    ...
    >
      ...
      <ResizablePanel defaultSize={80} className="flex-1 border bg-background rounded-lg">
        {schema && table && tableStore ? (
          <TableContent table={table} schema={schema} store={tableStore} />
        ) : (
