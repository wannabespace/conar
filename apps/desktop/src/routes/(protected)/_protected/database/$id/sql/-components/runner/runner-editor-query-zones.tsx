...
function RunnerEditorQueryZone({
  ...
  const index = useStore(store, (state) =>
    state.editorQueries.findIndex((query) => query.startLineNumber === lineNumber)
  )
  const { queriesLength, queryNumber } = useStore(store, (state) => {
    const queriesBefore =
      state.editorQueries.slice(0, index).reduce((sum, curr) => sum + curr.queries.length, 0) + 1
  ...
  return (
    <div
      className={cn('flex gap-2 items-center justify-between h-full', 'px-2 pr-6 py-1 border-y')}
    >
...
export function useRunnerEditorQueryZones(
  ...
  const store = databaseStore(database.id)
  const linesWithQueries = useStore(store, (state) =>
    state.editorQueries.map(({ startLineNumber }) => startLineNumber)
  )
