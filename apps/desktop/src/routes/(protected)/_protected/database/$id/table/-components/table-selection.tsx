...
export function SelectionHeaderCell({
  ...
  return (
    <div
      className={cn('flex items-center w-fit', columnIndex === 0 && 'pl-4', className)}
      style={{ width: `${size}px` }}
    >
...
export function SelectionCell({
  ...
  const isSelected = useStore(store, (state) =>
    state.selected.some((row) => keys.every((key) => row[key] === rows[rowIndex]![key]))
  )
          ...
          if (isSelected) {
            store.setState(
              (state) =>
                ({
                  ...state,
                  selected: store.state.selected.filter(
                    (row) => !keys.every((key) => row[key] === rows[rowIndex]![key])
                  ),
                }) satisfies typeof state
            )
          } else {
            store.setState(
              (state) =>
                ({
                  ...
                  selected: [
                    ...state.selected,
                    keys.reduce((acc, key) => ({ ...acc, [key]: rows[rowIndex]![key] }), {}),
                  ],
                }) satisfies typeof state
            )
          }
