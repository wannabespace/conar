...
function prepareValue(value: unknown, column?: Column): unknown {
  ...
  return typeof value === 'string' && column.isArray ? JSON.parse(value) : value
}
  ...
  return (
    <div className="sticky left-0 pointer-events-none h-full flex items-center pb-10 justify-center">
      <div className="flex flex-col items-center p-4 bg-card rounded-lg border max-w-md">
        ...
        <p className="text-sm font-mono text-center text-muted-foreground">{error.message}</p>
      </div>
    </div>
  )
}
...
function TableComponent({ table, schema }: { table: string; schema: string }) {
  ...
  } = useInfiniteQuery(databaseRowsQuery({ database, table, schema, query: { filters, orderBy } }))
  ...
  useEffect(() => {
    if (!rows || !store.state.selected) return
          ...
          ? {
              ...
              pages: data.pages.map((page, pageIndex) => ({
                ...
                rows: page.rows.map((row, rIndex) =>
                  pageIndex * data.pages[0]!.rows.length + rIndex === rowIndex
                    ? {
                        ...row,
                        [columnName]: value,
                      }
  ...
  const saveValue = useCallback(
    async (rowIndex: number, columnId: string, newValue: unknown) => {
      const rowsQueryOpts = databaseRowsQuery({
        database,
        table,
        schema,
      ...
      if (!data) throw new Error('No data found. Please refresh the page.')
      ...
      const updatedFilters = sqlFilters.map((filter) =>
        filter.column in modifiedColumns
          ? {
              ...filter,
      ...
      try {
        ...
        if (!result || !(columnId in result)) return

        const realValue = result[columnId]

        if (newValue !== realValue) setValue(rowIndex, columnId, realValue ?? undefined)
      } catch (e) {
        ...
        (column) =>
          ({
            ...
            size:
              ...
              (column.foreign ? 25 : 0),
            header: (props) => (
              <TableHeaderCell column={column} onSort={() => onOrder(column.id)} {...props} />
            ),
            cell: (props) => {
              const values = enums?.find(
                (e) =>
                  e.name === column.enum &&
