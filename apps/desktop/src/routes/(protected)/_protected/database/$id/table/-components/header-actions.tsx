...
export function HeaderActions({
  ...
  async function handleRefresh() {
    ...
    queryClient.invalidateQueries(
      databaseTableTotalQuery({ database, table, schema, query: { filters } })
    )
  ...
  const getLimitedData = async (limit: number) =>
    rowsQuery.run(database, {
      schema,
      table,
      limit,
      offset: 0,
  ...
  return (
    <div className="flex items-center gap-2">
      <HeaderActionsDelete table={table} schema={schema} database={database} />
      <HeaderActionsColumns database={database} table={table} schema={schema} />
      <HeaderActionsFilters />
      <HeaderActionsOrder database={database} table={table} schema={schema} />
      <TooltipProvider>
        <Tooltip>
