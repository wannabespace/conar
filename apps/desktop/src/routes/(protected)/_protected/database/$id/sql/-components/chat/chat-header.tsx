...
export function ChatHeader({ chatId }: { chatId: string }) {
  ...
  const { data: allChats } = useLiveQuery((q) =>
    q.from({ chats: chatsCollection }).orderBy(({ chats }) => chats.createdAt, 'desc')
  )
    ...
    <div className="flex justify-between items-center h-8 gap-2">
      ...
      <div className="flex items-center gap-2">
        {chat && (
          ...
          >
            <Link to="/database/$id/sql" params={{ id }}>
                  ...
                  <div key={group}>
                    ...
                    {chats.map((chat) => (
                      <DropdownMenuItem key={chat.id} asChild>
                        ...
                        >
                          {chat.title || (
                            <span className="animate-pulse bg-muted rounded-md w-30 h-4" />
                          )}
                        </Link>
                      </DropdownMenuItem>
                    ))}
