    ...
    >
      ...
      >
        <div className="size-12 shrink-0 rounded-lg p-3 bg-muted/70">
          <DatabaseIcon type={database.type} className="size-full" />
        </div>
        <div className="flex flex-1 flex-col min-w-0">
          <div className="font-medium tracking-tight truncate flex items-center gap-2">
            ...
            {database.label && <Badge variant="secondary">{database.label}</Badge>}
          </div>
          <div data-mask className="text-xs text-muted-foreground font-mono truncate">
...
export function Empty() {
  return (
    <div className="text-center bg-card border-2 border-dashed border-border/50 rounded-xl p-14 w-full m-auto group">
      <h2 className="text-foreground font-medium mt-6">No connections found</h2>
      <p className="text-sm text-muted-foreground mt-1 mb-4 whitespace-pre-line">
        Create a new connection to get started.
      </p>
      <Button asChild>
        <Link to="/create">Create a new connection</Link>
      </Button>
    </div>
  )
}
...
export function DatabasesList() {
  ...
  const availableLabels = Array.from(
    new Set(databases.map((db) => db.label).filter(Boolean) as string[])
  ).sort()
