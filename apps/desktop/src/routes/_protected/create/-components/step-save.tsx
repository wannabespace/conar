...
export function StepSave({
  ...
  const { data: connections } = useLiveQuery(q =>
    q.from({ connections: connectionsCollection }).orderBy(({ connections }) => connections.createdAt, 'desc'),
  )

  const existingLabels = connections.map(connection => connection.label).filter((l): l is string => l !== null)
      ...
      <div className="flex flex-col gap-6">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {COLOR_OPTIONS.map(colorOption => (
                <button
                  key={colorOption}
                  type="button"
                  className={cn('size-6 cursor-pointer rounded-full bg-(--color) transition-all', {
                    'ring-2 ring-(--color) ring-offset-2 ring-offset-background': color === colorOption,
                  })}
        ...
        <div className="flex flex-col gap-2">
          <label className="flex items-center gap-2 text-sm">
            <Checkbox checked={saveInCloud} onCheckedChange={() => setSaveInCloud(!saveInCloud)} />
