...
- **A className on a kit component is a missing prop.** Call sites pass `size`/`variant` and own layout only; re-dialing a kit component's surface (padding, radius, background, border, font size, height) means the kit needs a variant — hard rule in the `tamery-ui` skill.
- **Every feature ships a keyboard flow, and Escape always walks back to the Navigator** — one step per press (clear, then leave, then close), never a jump. Holding ⌘ reveals every shortcut on the controls that fire it. Hard rules in the `tamery-ui` skill.
