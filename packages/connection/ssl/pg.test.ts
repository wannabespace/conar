...
describe('SSL Configuration', () => {
  ...
  it('should parse sslmode=verify-full without sslrootcert', () => {
    const connectionString = parseConnectionString('postgresql://user:password@localhost:5432/mydb?sslmode=verify-full&sslcert=/path/to/cert.pem&sslkey=/path/to/key.pem')
    const config = parseSSLConfig(connectionString.searchParams)
  ...
  it('should throw error for sslmode=disable with SSL parameters', () => {
    ...
    expect(() => parseSSLConfig(connectionString.searchParams)).toThrow('sslmode=disable cannot be used with SSL certificate parameters (sslcert, sslkey, sslrootcert, sslpassword, sslservername)')
  })

  it('should parse sslmode=verify-full with SSL certificates', () => {
    const connectionString = parseConnectionString('postgresql://user:password@localhost:5432/mydb?sslmode=verify-full&sslcert=/path/to/cert.pem&sslkey=/path/to/key.pem&sslrootcert=/path/to/ca.pem')
    const config = parseSSLConfig(connectionString.searchParams)
  ...
  it('should parse SSL parameters without sslmode', () => {
    const connectionString = parseConnectionString('postgresql://user:password@localhost:5432/mydb?sslcert=/path/to/cert.pem&sslkey=/path/to/key.pem&sslrootcert=/path/to/ca.pem')
    const config = parseSSLConfig(connectionString.searchParams)
  ...
  it('should handle sslmode=require with SSL parameters (parameters should be merged)', () => {
    const connectionString = parseConnectionString('postgresql://user:password@localhost:5432/mydb?sslmode=require&sslcert=/path/to/cert.pem&sslkey=/path/to/key.pem')
    const config = parseSSLConfig(connectionString.searchParams)
